document.addEventListener("DOMContentLoaded", () => {
    let elementList = document.querySelectorAll('.ns__questionnaire__form__file__inp');
    if(elementList.length > 0) {
        elementList.forEach(function (inpFile) {
            inpFile.addEventListener('change', (e)=> {
                let label = inpFile.parentElement,
                    labelVal = label.querySelector('.ns__questionnaire__form__file__value');
                let filename = [];
                if(inpFile.files[0]) {
                    filename.push(inpFile.files[0].name);
                    if(inpFile.files[1])  filename.push(inpFile.files[1].name);
                    if(inpFile.files[2])  filename.push(inpFile.files[2].name);
                    labelVal.innerHTML = filename.join(', ');
                    labelVal.classList.add('active');
                }
            });
        });
    }

    const jsPass = document.querySelectorAll('.js-pass');

    jsPass.forEach(el => {
        el.addEventListener('click', (event) => {
            if (document.getElementById(el.dataset.target)) {
                let inputPass = document.getElementById(el.dataset.target);
                if (inputPass.type === 'password') {
                    inputPass.type = 'text';
                    el.classList.add('active');
                } else {
                    inputPass.type = 'password';
                    el.classList.remove('active');
                }
            }
        });
    });

    const maskPhone = document.querySelectorAll('.js-phone-mask');
    maskPhone.forEach(el => {
        let im = new Inputmask("+7 999 999-99-99", {"clearIncomplete": true});
        im.mask(el);
    });

    if(document.getElementById('revSlider')) {
        new Swiper('#revSlider', {
            speed: 600,
            navigation: {
                nextEl: '.swiper-next',
                prevEl: '.swiper-prev',
            },
            slidesPerView: 3,
            spaceBetween: 30,
            pagination: {
                el: '#revPag',
                clickable: true,
            },
            breakpoints: {
                300: {
                    slidesPerView: 1,
                    spaceBetween: 20

                },
                574: {
                    slidesPerView: 2,
                    spaceBetween: 20
                },
                991: {
                    slidesPerView: 3,
                    spaceBetween: 20
                },
                1200: {
                    slidesPerView: 3,
                    spaceBetween: 30
                }
            }
        });
    }

    const videoBackgrounds = new VideoBackgrounds('[data-vbg]');
});
